...
CKEDITOR.dialog.add( 'button', function( editor ) {
	function commitAttributes( element ) {
		...
		if ( val ) {
			...
			if (this.id == "type") {
			   element.attributes[ this.id ] = val == "save" ? "button" : val;
				if (val == "save") element.attributes[ "title" ] = "save";
				else delete element.attributes[ "title" ];
			   }
			else {
	...
	return {
		...
		onShow: function() {
			...
			if ( element && element.is( 'input' ) ) {
				...
				if ( type in { button: 1, reset: 1, save: 1, submit: 1 } ) {
					this.button = element;
         ...
         {//BASIC
            ...
            elements: [
               { type: 'hbox', widths: ['100%'], children: [
				...
				items: [
					[ 'Submit Form', 'submit' ],
					[ 'Save Form Progress', 'save' ],
					[ 'JavaScript Button', 'button' ],
					[ 'Reset Form', 'reset' ]
					],
				setup: function( element ) {
				   var v = element.getAttribute( 'type' );
					if (v == "button") {
					   var x = element.getAttribute( 'title' );
						if (x == "save") v = x;
						}
					this.setValue( v || '' );
					},
