...
$( document ).ready(function() {
    ...
    // var editor1 = $( "#editor1" );
	var CONTACTS = [
		{ type: 'Text', name: 'Lorem ipsum dolor sit amet dolor. Duis blandit vestibulum faucibus a, tortor.', avatar: 'hfin' },
		{ type: 'Image', name: 'Image.', src: "img/droparea.png", avatar: 'dartagnan' },
		{ type: 'Button', name: 'Button', avatar: 'pfog' }
	];
	
	CKEDITOR.disableAutoInline = true;
	CKEDITOR.config.extraPlugins = "lux_forms";
	
	CKEDITOR.plugins.add( 'lux_forms', {
		requires: 'dialog, widget, fakeobjects',
		lang: 'en',
		...
		init: function( editor ) {
			var lang = editor.lang,
				order = 0;
			editor.widgets.add( 'lux_forms', {
				// allowedContent: 'div[*]; span(!h-card); a[href](!u-email,!p-name); span(!p-tel); *[id]',
				requiredContent: 'div(lux_forms)',
				pathName: 'lux_forms',
				inline: true,
				upcast: function( el ) {
					return el.name == 'div' && el.hasClass( 'lux_forms' );
				}
			} );
			...
			editor.on( 'paste', function( evt ) {
				...
				switch(contact.type) {
					case "Text":
						evt.data.dataValue =
						'<div class="lux_forms" contenteditable="true" id="cke-txt-' + i + '">' +
							contact.name +
							' ' +
						'</div>';
						break;
					case "Image":
						evt.data.dataValue =
						'<div class="lux_forms" contenteditable="true" id="cke-txt-' + i + '" style="width:100%;height:50px;">' +
							'<img src="' + contact.src + '" style="width:30px; height:30px;">' +
							' ' +
						'</div>';
						break;
					case "Button":
	...
	// 	extraPlugins: 'lux_forms,sourcedialog,justify'
	// } );
});
