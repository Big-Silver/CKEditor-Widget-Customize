...
CKEDITOR.plugins.add( 'drag', {
	...
	init: function( editor ) {
		editor.widgets.add( 'drag', {
			allowedContent: 'input[id]; input[type]; textarea[id]; img[id]; select[id]; div[id]; span[id];' + 'span(!simplebox); div[id];',
		...
		editor.on( 'paste', function( evt ) {
			...
			switch(contact.type) {
				...
				case "HiddenField":
					...
					evt.data.dataValue =
					'<span class="h-card simplebox-content" contenteditable="true">' +
